...
async def start(update, context):
    ...
    if not exist:
        if ref_id and ref_id != uid:
            ...
            # 2. Update referrer's total referral count
            conn, c = get_db()
            c.execute("UPDATE users SET total_refers = total_refers + 1 WHERE user_id=?", (ref_id,))
            conn.commit(); conn.close()
            
            # 3. Message to new user
            await update.message.reply_text(
                "✅ **Referral Success!** 🎉\nYour referrer got **1 Credit**.",
                parse_mode="Markdown"
            )
            
            # 4. 🔥 NEW: Message to the referrer (real-time notification)
            try:
                ref_user = get_user(ref_id)
                ref_name = ref_user[1] if ref_user else str(ref_id)
                new_total = ref_user[5] if ref_user else 0  # total_refers
        ...
        update_credits(uid, 1)  # Bonus credit
...
async def leaderboard(update, context):
    if update.effective_user.id != ADMIN_ID and not await check_force_join(update, context): return
    top = get_top_referrers(10)  # Auto sorted by total_refers DESC
...
def main():
    ...
    print("🤖 Bot running with REFERRAL ALERT, AUTO LEADERBOARD & ADMIN FIX!")
