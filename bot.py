...
"""
🤖 Instagram Views Telegram Bot - ULTIMATE FIX
- Referral: 1 refer = 1 credit
- 1 credit = 500 views
- New user bonus: +1 credit
- Force join (admin exempt)
- Admin Panel (fully functional)
- Direct commands: /addcredits, /listusers, /setcredits, /deductcredits
"""
...
INVITE_LINKS = {
    -1004264165330: "https://t.me/+jA6VJIxg76NlYTA9",
    -1003967851938: "https://t.me/+C6Vq50Mp7r4xNDll"
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not existing:
        ...
        update_credits(user_id, 1)  # bonus
...
/orderstatus - Last order status
/checkjoin - Force join check
...
async def check_join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        if not await check_force_join(update, context):
            return
        else:
            await update.message.reply_text("✅ You have already joined all required chats!")
    else:
        await update.message.reply_text("✅ You are admin, exempt from force join.")
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        if not await check_force_join(update, context):
            return
    user_id = update.efficient_user.id
    user = get_user(user_id)
    if not user:
        await update.message.reply_text("❌ Please /start first.")
        return
...
# ===================== ADMIN PANEL (SIMPLIFIED & FIXED) =====================
# Conversation states – using simple numeric values
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_panel":
        ...
        await query.edit_message_text("📢 **Broadcast**\nSend the message you want to broadcast. (Cancel: /cancel)")
        return BROADCAST  # start broadcast conversation
        ...
        # Start conversation for credits
        action = data.split("_")[1]  # add, deduct, set
        context.user_data["credit_action"] = action
        await query.edit_message_text(f"Enter the **user_id** for {action}ing credits.\n(use /cancel to abort)")
        return ADD_CREDITS  # go to state ADD_CREDITS
    elif data.startswith("settings_"):
        setting = data.split("_")[1]  # api_key, service_id, api_url, views
        context.user_data["admin_setting"] = setting
        await query.edit_message_text(f"Enter the new **{setting.replace('_', ' ').title()}** :\n(use /cancel to abort)")
        return BROADCAST  # reuse broadcast state for settings
...
async def admin_credit_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle user_id input for credits."""
    text = update.message.text.strip()
    if not text.isdigit():
        await update.message.reply_text("❌ Please enter a valid numeric user_id. Try again or /cancel.")
        return ADD_CREDITS  # stay in same state
    user_id = int(text)
    user = get_user(user_id)
    if not user:
    ...
    await update.message.reply_text(f"Enter the amount of credits to **{context.user_data['credit_action']}** for user {user_id}:\n(/cancel to abort)")
    return AMOUNT  # go to amount state

async def admin_credit_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle amount input for credits."""
    text = update.message.text.strip()
    if not text.lstrip('-').isdigit():
        await update.message.reply_text("❌ Please enter a valid number. Try again or /cancel.")
        return AMOUNT
    amount = int(text)
...
async def admin_settings_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle settings change (API Key, Service ID, etc.)."""
    ...
    if setting == "api_key":
        ...
        if not text.isdigit():
            await update.message.reply_text("❌ Service ID must be a number. Try again or /cancel.")
            return BROADCAST
        ...
        if not text.isdigit():
            await update.message.reply_text("❌ Views per credit must be a number. Try again or /cancel.")
            return BROADCAST
...
async def list_users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    conn, cursor = get_db()
    cursor.execute("SELECT user_id, username, credits FROM users ORDER BY user_id DESC LIMIT 20")
    users = cursor.fetchall()
    conn.close()
    if not users:
        ...
        return
    msg = "👥 **Last 20 users:**\n\n"
    for uid, username, credits in users:
        msg += f"🆔 `{uid}` – @{username or 'N/A'} – Credits: {credits}\n"
    await update.message.reply_text(msg, parse_mode="Markdown")

async def deduct_credits_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("⛔ Unauthorized.")
        return
    try:
        ...
        await update.message.reply_text("❌ Usage: `/deductcredits <user_id> <amount>`", parse_mode="Markdown")

async def set_credits_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("⛔ Unauthorized.")
        return
    try:
        target_id = int(context.args[0])
        amount = int(context.args[1])
        set_credits(target_id, amount)
        await update.message.reply_text(f"✅ Set credits to **{amount}** for user `{target_id}`.", parse_mode="Markdown")
    except (IndexError, ValueError):
        await update.message.reply_text("❌ Usage: `/setcredits <user_id> <amount>`", parse_mode="Markdown")
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(admin_callback, pattern="^settings_.*$"),
        ],
        states={
            ADD_CREDITS: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_credit_user_id)],
            AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_credit_amount)],
            BROADCAST: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_broadcast_message)],
            # We reuse BROADCAST for settings as well, but we need to differentiate.
            # We'll use a different state for settings to avoid conflict.
            # Actually we can just use the same state but check context.
            # For simplicity, let's add another state for settings.
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    app.add_handler(conv_handler)

    # Separate handler for settings to avoid conflict
    settings_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(admin_callback, pattern="^settings_.*$")],
        states={
            BROADCAST: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_settings_input)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    app.add_handler(settings_conv)

    print("🤖 Bot is running with COMPLETE FIX!")
