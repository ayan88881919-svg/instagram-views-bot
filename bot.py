...
def update_referral_count(uid):
    conn, c = get_db()
    c.execute("UPDATE users SET total_refers = total_refers + 1 WHERE user_id=?", (uid,))
    conn.commit(); conn.close()
...
async def check_force_join(update, context):
    ...
    if not_joined:
        ...
        await context.bot.send_message(
            ...
            f"⚠️ **You must join the following chats:**\n" + 
...
def place_smm_order(link, qty=VIEWS_PER_CREDIT):
    try:
        ...
        j = r.json()
        if "order" in j:
            return True, str(j["order"]), "Ok"
        elif j.get("status")=="success" and "order_id" in j:
            return True, str(j["order_id"]), "Ok"
        else:
            return False, None, j.get("error","Unknown")
...
async def start(update, context):
    # ✅ Admin check pehle - force join skip
    if update.effective_user.id != ADMIN_ID and not await check_force_join(update, context):
        return
    ...
    if context.args:
        ...
        ref_id = get_user_by_code(ref_code)
    if not exist:
        if ref_id and ref_id != uid:
            update_credits(ref_id, 1)
            add_referral(ref_id, uid)
            update_referral_count(ref_id)
            await update.message.reply_text(
                "✅ **Referral Success!** 🎉\nYour referrer got **1 Credit**.",
                parse_mode="Markdown"
            )
            try:
                updated_user = get_user(ref_id)
                new_total = updated_user[5] if updated_user else 0
                await context.bot.send_message(
                    ref_id,
                    f"🎉 **New Referral Alert!**\n\n@{uname} just joined using your referral link!\nYou earned **1 Credit** 💰\nTotal Referrals: **{new_total}**\n\nKeep sharing! 🚀",
                    parse_mode="Markdown"
                )
            except Exception as e:
                logger.error(f"Notify failed: {e}")
        new_code = generate_code()
        add_user(uid, uname, new_code, ref_id)
...
"""
        ...
        credits=exist[4]
        refs=exist[5]
        msg = f"""👋 **Welcome back {uname}!**

🔹 Code: `{code}`
🔹 Referrals: {refs}
🔹 Credits: {credits}

➡️ Send `/redeem <link>` to get {VIEWS_PER_CREDIT} views.
...
async def help_command(update, context):
    if update.effective_user.id != ADMIN_ID and not await check_force_join(update, context):
        return
    help_text = f"""📖 **Help - Refer & Earn Bot**
...
/balance – Check your credits & referrals
/redeem <link> – Redeem 1 credit for {VIEWS_PER_CREDIT} views
/leaderboard – Top 10 referrers
/orderstatus – Check last order status
/checkjoin – Check if joined required chats
/help – Show this help

🤖 **Admin Commands:**
/admin_panel – Open admin panel (only for admin)
/addcredits <user_id> <amount> – Add credits
/deductcredits <user_id> <amount> – Deduct credits
/setcredits <user_id> <amount> – Set credits
/listusers – Show last 20 users

⚠️ *Automated views may violate Instagram ToS. Use at your own risk.*
"""
...
async def check_join_command(update, context):
    if update.effective_user.id != ADMIN_ID:
        ...
        await update.message.reply_text("✅ Admin exempt.")

async def refer(update, context):
    # ✅ Admin skip force join
    if update.effective_user.id != ADMIN_ID and not await check_force_join(update, context):
        return
    uid = update.effective_user.id
    u = get_user(uid)
...
async def redeem(update, context):
    ...
    if credits <= 0:
        await update.message.reply_text(
            f"❌ Insufficient credits! Need 1 Credit for {VIEWS_PER_CREDIT} views. Use /refer to earn.",
            parse_mode="Markdown"
        )
        return
    update_credits(uid, -1)
    ...
    if success:
        ...
        await update.message.reply_text(
            f"✅ **Success!** {VIEWS_PER_CREDIT} views being sent to:\n`{link}`\n🆔 Order ID: `{oid}`\n💰 Remaining Credits: {credits-1}\n⏳ Delivery 5-30 min.",
            parse_mode="Markdown"
        )
        await context.bot.send_message(
            ADMIN_ID,
...
async def admin_callback(update, context):
    ...
    if data == "admin_panel":
        ...
        kb = [
            [InlineKeyboardButton("➕ Add", callback_data="credits_add")],
            [InlineKeyboardButton("➖ Deduct", callback_data="credits_deduct")],
            [InlineKeyboardButton("🔧 Set", callback_data="credits_set")],
        ...
        kb = [
            ...
            [InlineKeyboardButton("🆔 Service ID", callback_data="settings_service_id")],
            [InlineKeyboardButton("🌐 API URL", callback_data="settings_api_url")],
            [InlineKeyboardButton("📊 Views/Credit", callback_data="settings_views")],
            [InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]
        ]
        ...
        oid = data.split("_")[2]
        update_redeem_status(oid, "refunded")
        ...
        await q.edit_message_text(f"Enter new {setting.replace('_',' ').title()}: (or /cancel)")
        return BROADCAST
...
async def show_user_detail(update, context, uid):
    ...
    kb = [
        [InlineKeyboardButton("➕ Add", callback_data=f"user_addcredits_{uid}")],
        [InlineKeyboardButton("➖ Deduct", callback_data=f"user_deductcredits_{uid}")],
        [InlineKeyboardButton("🔧 Set", callback_data=f"user_setcredits_{uid}")],
...
async def show_order_detail(update, context, oid):
    ...
    text = f"📋 **Order #{oid_db}**\n🆔 ID: {oid2}\n👤 User: {uid}\n🔗 {link}\n📊 Views: {views}\n📌 Status: {st}\n📅 {created}"
    kb = [
        [InlineKeyboardButton("🔄 Check", callback_data=f"order_check_{oid2}")],
        [InlineKeyboardButton("💰 Refund", callback_data=f"refund_order_{oid2}")],
...
async def admin_settings_input(update, context):
    ...
    if setting == "api_key":
        ...
        await update.message.reply_text(f"✅ Service ID: {SMM_SERVICE_ID}")
    elif setting == "api_url":
        SMM_API_URL = text
        await update.message.reply_text(f"✅ API URL: {SMM_API_URL}")
    elif setting == "views":
        if not text.isdigit():
            await update.message.reply_text("❌ Must be number.")
            return BROADCAST
        VIEWS_PER_CREDIT = int(text)
        await update.message.reply_text(f"✅ Views per Credit: {VIEWS_PER_CREDIT}")
    context.user_data.clear()
    return ConversationHandler.END
...
async def list_users_cmd(update, context):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    conn, c = get_db()
    c.execute("SELECT user_id, username, credits FROM users ORDER BY user_id DESC LIMIT 20")
    users = c.fetchall()
    conn.close()
    if not users:
    ...
    for uid, uname, cred in users:
        msg += f"🆔 {uid} – @{uname or 'N/A'} – Credits: {cred}\n"
    await update.message.reply_text(msg, parse_mode="Markdown")
...
async def deduct_credits_cmd(update, context):
    ...
    try:
        ...
        amt = int(context.args[1])
        update_credits(uid, -amt)
        await update.message.reply_text(f"✅ Deducted {amt} credits from {uid}.")
    except:
...
async def set_credits_cmd(update, context):
    ...
    try:
        ...
        amt = int(context.args[1])
        set_credits(uid, amt)
        await update.message.reply_text(f"✅ Set credits to {amt} for {uid}.")
    except:
...
def main():
    ...
    print("🤖 Bot running with REFERRAL & LEADERBOARD FIXED!")
