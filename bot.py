...
def update_referral_count(uid):
    """Increase total_refers by 1 for the referrer."""
    conn, c = get_db()
...
async def start(update, context):
    ...
    if not exist:
        # 🔥 NEW USER: Handle referral
        if ref_id and ref_id != uid:
            # 1. Give credit to referrer
            update_credits(ref_id, 1)
            # 2. Add referral to history
            add_referral(ref_id, uid)
            # 3. ✅ FIX: Update referrer's total_refers count
            update_referral_count(ref_id)
            
            # 4. Message to new user
            ...
            try:
                ...
                ref_name = ref_user[1] if ref_user else str(ref_id)
                # Get updated total
                ...
                logger.error(f"Could not notify referrer {ref_id}: {e}")
        
        # Register new user
        new_code = generate_code()
        add_user(uid, uname, new_code, ref_id)
        update_credits(uid, 1)  # Bonus
...
async def order_status(update, context):
    ...
    if not r: await update.message.reply_text("No orders found."); return
    oid, st, link = r
...
def main():
    ...
    print("🤖 Bot running with REFERRAL FIXED & LEADERBOARD AUTO UPDATED!")
