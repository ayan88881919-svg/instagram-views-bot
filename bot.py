...
"""
🤖 Instagram Views Telegram Bot - Complete (Admin Panel FULLY FIXED)
- Referral: 1 refer = 1 credit
- 1 credit = 500 views (user sends Reel link)
- New user bonus: +1 credit
- Force join (1 channel + 1 group) - Admin exempt
- Admin panel via /admin_panel (only admin)
- Direct commands: /addcredits, /listusers
"""
...
FORCE_JOIN_CHATS = [
    -1004264165330,  # Channel ID
    -1003967851938   # Group ID
]
...
/checkjoin - Check if you joined all required chats
...
# ---------- DIRECT ADMIN COMMANDS (for quick use) ----------
...
async def list_users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Direct command: /listusers - show last 20 users"""
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("⛔ Unauthorized.")
        return
    conn, cursor = get_db()
...
def main():
    ...
    print("🤖 Bot is running with FULLY FIXED Admin Panel!")
